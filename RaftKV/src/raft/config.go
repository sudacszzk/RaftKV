...
func (cfg *config) nCommitted(index int) (int, interface{}) {
	...
	for i := 0; i < len(cfg.rafts); i++ {
		...
		DPrintf2("index in ncommitted: %v",index)
		//DPrintf2("logs: %v",cfg.logs)
		cfg.mu.Unlock()
		DPrintf2("ggg %v",cmd1)
		//DPrintf2("%v follower",rf.logs)
		if ok {
			if count > 0 && cmd != cmd1 {
